  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F8F7]">
      <header className="h-16 shrink-0 flex items-center justify-between px-4 sm:px-8 bg-white border-b border-[#D5E2DF]">
        <h1 className="text-xl font-semibold text-[#173F3A]">Documents</h1>
        <div className="flex space-x-4">
          <button
            onClick={() => setIsUploadOpen(true)}
            className="px-4 py-2 bg-[#23796F] text-white text-xs font-bold rounded-lg hover:bg-[#173F3A] transition-colors uppercase tracking-widest"
          >
            Upload Document
          </button>
        </div>
                  ...
                  filteredDocs.map((doc) => (
                    <tr key={doc.id} className="hover:bg-gray-50/50">
                      <td className="px-6 py-3">
                        <div className="font-medium text-[#173F3A] text-xs">{doc.member?.full_name}</div>
                        <div className="text-[10px] text-gray-500">{doc.member?.member_code}</div>
                      </td>
                      <td className="px-4 py-3 text-xs font-medium text-[#173F3A] uppercase tracking-wide">
                        {doc.document_type}
                      </td>
                      ...
                      <td className="relative px-4 py-3 text-right text-[10px] font-bold uppercase tracking-widest sm:pr-6">
                        <button className="text-[#23796F] hover:text-[#173F3A] transition-colors">
                          View
                        </button>
                      </td>
                    </tr>
